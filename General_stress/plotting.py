...
def plotenzyme(dfallcells,numberoftrench,treated,duration):
    
        colors=   mcp.gen_color(cmap="nipy_spectral",n=numberoftrench+1)  #['k', 'limegreen', 'turquoise', 'dodgerblue', 'midnightblue']
                ...
                for trench in range(   numberoftrench):
                    ...
                    if len(df2)>0:
                        ...
                        for postrench in range(0,1):
                                ...
                                sx2= df_mothergr.Enzyme.values
                                ax.plot(t-treated, sx2 , lw=1,  label='GrxA' , color=colors[trench])
                         
                                ax.set_xlabel('Time', fontsize=15)
                                ax.set_ylabel('Enzyme', fontsize=15 )   
                                ax.set_title('Trench number = %s'%trench)
                ...
                for trench in range(   numberoftrench):
                    ...
                    if len(df2)>0:
                        ...
                        for postrench in range(0,1):#int(max(df.max_cells))):
                                df_mother = df2[df2.PositionClosedEnd ==  postrench]
                                df_mother=df_mother[df_mother.Time>treated +400]
                                df_mothergr =  df_mother.groupby('Frame').mean()
                              
                                t=df_mothergr.index 
                      
                                sx2= df_mothergr.Enzyme.values
                                sx2= sx2[treated+1000:]
                                ax.plot(sx2[10:], sx2[:-10] , lw=1,  label='GrxA' , color=colors[trench])
                         
                                ax.set_xlabel('Enzyme at t+10', fontsize=15)
                                ax.set_ylabel('Enzyme', fontsize=15 )          
                                ax.set_title('Trench number = %s'%trench)

def plotenzymegrad(dfallcells,numberoftrench,treated,duration):
   colors=   mcp.gen_color(cmap="nipy_spectral",n=numberoftrench+1) 
   image,ax =plt.subplots(figsize=(2,2), dpi=300)
