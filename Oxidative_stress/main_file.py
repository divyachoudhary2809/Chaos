...
#@jit(nopython=True)
def main_run (     limm, gI, trenches_, duration, conx,time_oftreat):
    ...
    rc = 1.15/2.  ;w= 1.2
    bH = 0.02 ; OxyRtot =1.;bGrxA = 0.00;bKatG= 0.00;bAhpC =0.01*1.;
                 ...
                 for trenchNum in range(len(trenches)):      
                     ...
                     for cell in range(len(lenofcells)): 
                            ...
                            if lx>0:
                                     ...
                                     ygg_cells[lx][trenchNum][cell] = odeint(response_model.comb_model3_growth,ygg_cells[lx-1][trenchNum][cell] ,timee,args=(tp,[c,c], gI , kgi, cI ))
                            else:
                                    c = c0init[lx]
                                    ygg_cells[lx][trenchNum][cell] = odeint(response_model.comb_model3_growth,ygg_cells[lx][trenchNum][cell],timee,args=(tp,[c,c] ,gI , kgi, cI))
    ...
    y0x=[bH,OxyRtot,bGrxA,bKatG,bAhpC] 
    yggput = odeint(response_model.comb_model3_growth,y0x,timee,args=([i for i in range(0,nfr)],concallt[0:nfr] ,gI, kgi, cI))  
    ...
    gett_2 = modelfunc.tps2([trenches ] , duration, [i * 150 for i in range(0,numberoftrench) ] , gett_,alllengths,cellnumberarray,alllgrowths,grxaall ,h2o2_,oxyRall,katgall,ahpcall,numberoftrench)
    arrays=[[]]
